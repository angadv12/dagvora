...
One hundred three tests pass: fifteen graph and schema, nineteen execution state, thirty-one scheduler, nineteen proposal queue and handle, and nineteen scheduler proposal drain. Scheduler and proposal tests use `asyncio.Event` for ordering and run through `asyncio.run` from sync test functions; there is no sleep-based sequencing and no async test plugin.
